...
**`SHEET_ID`를 줬는데 시트를 못 읽으면 빌드가 실패한다.** 일부러 그렇게 해 뒀다.
시드로 대체하고 빌드를 성공시키면 사이트가 예시 41건짜리로 조용히 바뀌어서,
"시트에 써도 안 바뀐다"로만 보인다. 빌드가 실패하면 Vercel은 마지막 성공 배포를
계속 내보내므로 사이트는 죽지 않고, 실패는 눈에 보인다.
...
| 이름 | 쓰임 |
|---|---|
| `SHEET_ID` | 자료 시트 문서 ID. 비우면 시드 CSV로 빌드. **주면 못 읽을 때 빌드 실패** |
| `ALLOW_SEED_FALLBACK` | `1`이면 시트를 못 읽어도 시드로 빌드한다. 권장하지 않음 |
