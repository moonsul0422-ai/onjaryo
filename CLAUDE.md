...
├── public/
│   ├── covers/                 # 자료 표지. 시트 coverImage 열이 파일 이름을 가리킨다
│   ├── robots.txt
│   ├── favicon.svg
│   └── og-default.png
└── src/
...
```ts
{
  ...
  tags: string[]
  coverImage: string | null    // "/covers/<파일이름>" — 시트에는 파일 이름만 적는다
...
### `index.astro`
**첫 화면은 리모컨이다.** 학년·과목·주제 버튼을 누르면 페이지를 옮기지 않고 바로
아래 목록이 그 자리에서 좁혀진다. 누를 때마다 새 화면이 열리면 찾는 일이 아니라
돌아다니는 일이 된다.

- 히어로: 한 줄 소개 + 검색창. 검색창은 그대로 `/search` 로 보낸다.
- 그 아래 버튼판: 학년(학교급별로 줄 나눔) / 과목 / 주제. 자료가 있는 값만 낸다.
- 그 아래 자료 목록. `ResourceCard`(compact 아님)라 표지가 함께 나온다.
- **같은 줄 안에서는 또(OR), 줄이 다르면 그리고(AND).** "초3·초4 를 누르고 수학" 이
  사람이 기대하는 뜻이다.
- 버튼 옆 숫자는 **지금 조건에서 이걸 더 누르면 몇 건인지**를 그때그때 다시 센다.
  0 이 되는 버튼은 흐리게 둔다. 빈 화면으로 들어가는 길을 만들지 않기 위함이다.
- 고른 조건은 `?g=&s=&p=` 로 `replaceState` 한다. 뒤로가기를 망가뜨리지 않으면서
  좁힌 화면을 그대로 보낼 수 있다.
- 목록은 12건씩 끊어 보여주고 "더 보기"로 이어 붙인다.
- **목록은 전부 서버에서 그려 두고 스크립트는 보이고 숨기기만 한다.** 카드를
  클라이언트에서 만들면 색인에 아무것도 남지 않고, JS 를 끈 사람에게는 빈 페이지가
  된다. JS 가 없으면 전부 보이고 버튼은 학년·과목·주제 페이지로 가는 링크가 된다.
- 자료가 몇백 건을 넘어가면 첫 로딩이 무거워진다. 그때는 이 목록을 검색 색인에서
  그리는 쪽으로 옮긴다.
- 하단에 총 등록 건수와 마지막 갱신일.

### `grade/[grade].astro`, `grade/[grade]/[subject].astro`
- 과목 탭(`.subjtab`)은 학년 페이지의 유일한 다음 클릭지다. 홈의 학년 버튼보다
  한 단계 크게 두고, 아래에 선을 그어 목록과 구역을 나눈다.
- 두 페이지가 같은 탭 줄을 쓴다. "전체"가 맨 앞에 오고 현재 과목이 눌린 상태가 된다.
- 과목을 고르지 않아도 목록은 바로 보인다.

### 표지 이미지
- `ResourceCard`(compact 아님)와 상세 페이지 상단 밴드가 `Cover.astro` 를 쓴다.
- `coverImage` 가 비면 기관 코드에서 뽑은 색 카드를 그린다. 자료 대부분은 한동안
  표지가 없을 것이고, 회색 네모가 줄줄이 늘어선 목록은 안 만드느니만 못하다.
- 시트에는 **파일 이름만** 적는다. 외부 주소와 경로 구분자는 `fetch-sheet.mjs` 가 막는다.
  남의 서버 이미지는 그쪽이 파일을 옮기면 깨지고, 시트 값이 그대로 URL 이 되기 때문이다.
- `public/covers/` 에 파일이 없으면 경고를 남기고 색 카드로 둔다. 빌드는 멈추지 않는다.
...
### `submit.astro`
- **받는 것은 원문 주소 하나다.** 기관·영역·설명 칸을 늘어놓으면 제보가 숙제가 되고,
  그 칸들은 어차피 등록할 때 원문을 열어 다시 채운다.
- 구글 폼(`PUBLIC_SUBMIT_FORM_URL`)이 연결돼 있으면 임베드한다. 폼도 "원문 주소"
  한 칸짜리로 만든다.
- 폼이 없으면 붙여넣기 칸 하나와 메일 앱을 여는 버튼을 둔다. 받는 주소는 화면에
  그대로 적어 두어 메일 앱이 없어도 막히지 않는다.
- **자체 폼과 서버를 만들지 않는다.** 제보를 받아 저장하는 엔드포인트를 두는 순간
  스팸과 개인정보를 떠안는다.
...
- **시트 작성 규칙은 `docs/sheet-rules.md`.** 어떤 값이 그 줄을 통째로 날리는지,
  붙여넣은 직후 무엇을 확인하는지가 들어 있다. 프롬프트(`curation-prompt.md`)는
  Claude 에게 주는 것이고, 이 문서는 사람이 보는 것이다.
- 분류값을 추가하면 `npm run prompt -- --write`로 프롬프트를 다시 뽑는다.
- 시트가 바뀌어도 정적 사이트는 다시 빌드해야 반영된다. 매일 05:20 KST 자동.
- **배포한 건수가 시트와 다르면 데이터 원천부터 본다.** `SHEET_ID` 가 없거나 시트를
  읽지 못하면 `scripts/seed/*.csv`(예시 41건)로 빌드된다. 표는 푸터의 `예시 데이터`
  배지와 `meta.source` 다. 빌드 로그에도 크게 찍는다.
  실서비스 배포(`VERCEL_ENV=production`)는 이때 아예 실패시킨다 — 예시 데이터가
  그대로 올라가는 쪽이 더 나쁘다. 급하면 `ALLOW_SEED=1`.
- Vercel 에 넣을 환경변수: `SHEET_ID`, 자료 탭이 첫 탭이 아니면
  `SHEET_RESOURCES_GID`, 기관 탭이 `organizations` 가 아니면 `SHEET_ORGS_GID`.
  시트는 "링크가 있는 모든 사용자에게 공개" 여야 한다 — 비공개면 CSV 대신
  로그인 HTML 이 와서 시드로 떨어진다.
