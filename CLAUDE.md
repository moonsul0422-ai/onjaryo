...
| 시트를 못 읽으면 | **빌드를 세운다.** 시드로 갈아 끼우면 예시 41건이 조용히 배포된다 |
| DB | 없음. 빌드 시 JSON 생성 |
...
- 파일 업로드·호스팅 기능
- 회원가입, 로그인, 즐겨찾기, 댓글, 별점
- **자료 제보 창구** (제보 폼·업로드·접수 페이지). 등록은 운영자가 직접 확인한 것만 올린다.
  소개 페이지의 문의용 메일 주소는 오류·끊긴 링크를 받는 곳이지 자료를 받는 곳이 아니다
- 교육청 사이트 크롤링
- 원문 텍스트를 요약란에 복사
- 광고 코드 삽입
...
- **시트를 못 읽었는데 시드 데이터로 빌드를 성공시키기.** 사이트는 멀쩡해 보이고
  내용만 예시로 바뀌어, 원인을 찾는 데 며칠이 걸린다. 차라리 빌드를 세운다
...
**빈 결과**: "찾는 자료가 없습니다"로 끝내지 말고, 다시 찾는 방법과 분류 목록을 함께 보여준다.
