...
- **학년 그리드 하나만 둔다.** 학교급별로 줄을 나눈다. 13개를 한 줄에 늘어놓지 않는다.
- 과목·주제·기관 칩과 "많이 찾는 자료"는 첫 화면에 두지 않는다. 고를 것을 다 펼치면
  정작 첫 클릭이 늦어진다. 과목은 학년을 고른 다음 화면에서, 나머지는 검색에서 만난다.
- **최근 등록 10건은 표지와 함께 남긴다.** `ResourceCard`(compact 아님)를 그대로 쓴다.
  학년 버튼만 놓인 첫 화면은 여기에 무엇이 있는 곳인지 보여 주지 못한다.
  표지가 붙은 자료 몇 건이 그 일을 한다.
